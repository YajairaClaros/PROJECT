...
class Ver_Maestros:
    ...
    def mostrar(self):
        ttk.Label(self.content_frame, text="Maestros", font=("Bookish", 16, "bold")).grid(row=1, column=0, columnspan=2, pady=(0, 10))

        # ---------------------- Entradas ----------------------
        self.Frame_entry = ttk.Frame(self.content_frame, style="White.TFrame")

        label_nombre = ttk.Label(self.Frame_entry, text="Nombre del maestro:")
        ...
        button_agregar_maestro.grid(row=0, column=2, pady=(20, 0), padx=(10, 0))

        self.Frame_entry.grid(column=0, row=2, columnspan=2)

        # ---------------------- Tabla Treeview ----------------------
        columnas = ("Nombre", "Opciones")
